...
//static int mmc3416x_i2c_detect(struct i2c_client *client, int kind, struct i2c_board_info *info);
 int mmc_probe(struct platform_device *pdev);
 int mmc_remove(struct platform_device *pdev);
...
/*----------------------------------------------------------------------------*/
 ...
 struct platform_driver mmc_sensor_driver = {
	.probe      = mmc_probe,
	.remove     = mmc_remove,    
	.driver     = {
		.name  = "msensor",
...
static int mmc3416x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	{
		printk(KERN_ERR "mmc3416x_device set TM cmd failed\n");
		goto exit_kfree;
	}
...
//	m_sensor_deteced = true;
	printk("mmc3416X IIC probe successful !");
	return 0;
...
{
	i2c_register_board_info(3, &i2c_mmc3416x, 1);
