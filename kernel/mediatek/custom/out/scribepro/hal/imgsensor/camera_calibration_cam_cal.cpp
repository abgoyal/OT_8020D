...
#define GODEN_RGAIN 598
#define GODEN_GGAIN 1020
#define GODEN_BGAIN 624
//[End]
...
static int GetAWBGain(u16* rGain, u16* gGain, u16* bGain, u8* otpData, u16 bufSize);
//[End]
...
const CALIBRATION_LAYOUT_STRUCT CalLayoutTbl[MAX_CALIBRATION_LAYOUT_NUM]=
{
	...
	{//CALIBRATION_LAYOUT_SENSOR_OTP
		...
		{		
			...
			{0x00000001, 0x00000000, 0x0000000E, DoCamCal2AGain}, //CAMERA_CAM_CAL_DATA_3A_GAIN
			{0x00000000, 0x00000000, 0x00000000, DoCamCal3DGeo}  //CAMERA_CAM_CAL_DATA_3D_GEO
		}
	}
    //[End]
...
static int GetAWBGain(u16* rGain, u16* gGain, u16* bGain, u8* otpData, u16 bufSize)
{
	...
	{
		CAM_CAL_LOG_IF(dumpEnable,"checksum NOK! checksum = %d, *(otpData+1) = %d\n", checksum, *(otpData+1));
		return 0;
	}
...
UINT32 DoCamCal2AGain(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
    ...
    { 
        ...
        else
        {
            ...
            ret = CamCalReadOTP(CamcamFID, start_addr, &awbBuf[0], 42);

			if(1 == ret)
			{
                err = CAM_CAL_ERR_NO_ERR;
                ret = GetAWBGain(&rGain, &gGain, &bGain, &awbBuf[0], 42);
                if(1 == ret)
                {
                    err = CAM_CAL_ERR_NO_ERR;
