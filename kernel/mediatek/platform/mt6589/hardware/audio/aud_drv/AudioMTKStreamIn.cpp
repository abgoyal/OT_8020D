...
#if 1//added by jrd.lipeng for only using main mic data for recording as default.		
    property_set("streamin.micchoose", "1");// 1: main mic; 2: sub-mic; 0: both
#endif
#endif
...
//specific MIC choose or LR channel Switch
    ...
    short * tmp_buf = buffer;//added by jrd.lipeng for a mistake of mtk original code.
...
#endif
    {
        buffer = tmp_buf;//added by jrd.lipeng for a mistake of mtk original code.
        while(copysize)
        {
            left = *(buffer);
            right = *(buffer+1);
