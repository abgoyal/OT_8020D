...
#if 1//Added by jrd.lipeng for EN60950
{
    String8 key = String8("GetNotifyVolumeIndex");
    if (param.get(key, value) == NO_ERROR) {
        value = "10,10";//fm, music. These values are the max safe steps, when exceed this step, give the notification.
        param.remove(key);
        returnParam.add(key, value);
        goto EXIT_GETPARAMETERS;
    }
}
#endif
...
// FM Control
////////////////////////////////////////////////////////
// PR419270-yiliang.kong---Begin
static bool mFMFirstOpen = true;
// PR419270-yiliang.kong---End
status_t AudioMTKHardware::SetFmEnable(bool enable)
{
	...
	if (mFMFirstOpen && !fmVolume) {
	    mAudioVolumeInstance->SetFmVolume(4);
	    mFMFirstOpen = false;
	}
	else {
	    mAudioVolumeInstance->SetFmVolume(fmVolume);
...
status_t AudioMTKHardware::SetFmDigitalEnable(bool enable)
{
    ...
    if (enable == true) { // enable FM neccessary function
        ...
        mAudioResourceManager->EnableAudioLock(AudioResourceManagerInterface::AUDIO_HARDWARE_LOCK, 3000);
	// PR457768-yiliang.kong---Begin
	bool forceStopOutputDevice = false;
	if (mFMFirstOpen && IsOutPutStreamActive() == true) {
		mAudioResourceManager->StopOutputDevice();     //Close Analog Device
		forceStopOutputDevice = true;
	}
	// PR457768-yiliang.kong---End
        mAudioVolumeInstance->SetFmVolume(fmVolume);
        SetFmDirectConnection(true,true);
	// PR457768-yiliang.kong---Begin
	if (mFMFirstOpen == true) {
	    mFMFirstOpen = false;
	    sleep(1); //Waiting for Fm Digit ready
	}
	// PR457768-yiliang.kong---End
        if (IsOutPutStreamActive() == false || forceStopOutputDevice == true) {
            mAudioResourceManager->StartOutputDevice();                 //Open Analog Device
        }
    }
    else if (enable == false) {
