...
init()
{
    ...
    m_i4preexp=0;
    m_i4preafe=0; 
    m_i4preScenario1=SENSOR_CMD_MAX;
    m_i4preScenario2=SENSOR_CMD_MAX;
...
setSensorExpTime(MUINT32 a_u4ExpTime)
{
    ...
    halSensorCmd_e	sensorScenario;
    
    err = m_pSensorHalObj->sendCommand(m_eSensorDevId, SENSOR_CMD_GET_SENSOR_SCENARIO, (int)&sensorScenario, 0, 0);
    if(err) {
        MY_ERR("Err CMD_SENSOR_SET_EXP_TIME, Sensor dev:%d\n", m_eSensorDevId);
    }
    if(m_i4preexp==a_u4ExpTime && m_i4preScenario1==sensorScenario)
        return err;
    m_i4preexp=a_u4ExpTime;
    m_i4preScenario1=sensorScenario;
...
setSensorGain(MUINT32 a_u4SensorGain) 
{
    ...
    if(m_i4preafe==a_u4SensorGain && m_i4preScenario2==sensorScenario)
        return err;
    m_i4preafe=a_u4SensorGain;
    m_i4preScenario2=sensorScenario;
