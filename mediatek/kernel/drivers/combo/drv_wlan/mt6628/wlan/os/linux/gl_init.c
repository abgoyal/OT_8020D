...
BOOLEAN fgHasPendingInterrupt = false;
    ...
    )
{
    ...
    do {
		...
		/* ensure waitq is initialized before prGlueInfo is set into HifInfo */
...
bailout:
	...
	if(TRUE == prAdapter->fgEnable5GBand)
		...
		if (fgHasPendingInterrupt){
			set_bit (GLUE_FLAG_INT_BIT, &prGlueInfo->u4Flag);
			fgHasPendingInterrupt = false;
		}
